...
public class UpgradeEVPGrantRequest extends UpgradeProcess {
	...
	protected EVPGrantRequest addEVPGrantRequest(
		...
		if (ArrayUtil.contains(
				EVPWorkflowConstants.GRANT_STATUSES_COMPLETE, status)) {

			evpGrantRequest.setStatus(status);

			long statusByUserId = (Long)statusFields[1];

			evpGrantRequest.setStatusByUserId(statusByUserId);

			String statusByUserName = (String)statusFields[2];

			evpGrantRequest.setStatusByUserName(statusByUserName);

			Date statusDate = (Date)statusFields[3];

			evpGrantRequest.setStatusDate(statusDate);
		}
		else {
			StringBundler sb = new StringBundler(6);

			sb.append("Grant request ");
			sb.append(evpGrantRequest.getEvpGrantRequestId());
			sb.append(" for user ");
			sb.append(evpGrantRequest.getUserName());
			sb.append(" must be manually workflowed to state ");
			sb.append(EVPWorkflowConstants.getGrantStatusLabel(status));
	...
	protected void updateEVPGrantRequests() throws Exception {
		...
		try {
			...
			ps = con.prepareStatement("select * from evp_GeneralGrantRequest");
			...
			while (rs.next()) {
				...
				if (evpGrantRequest.getStatus() <= 0) {
					...
					HashMap<String, Object> evpGrantRequestContext =
					...
					workflowContext.put(
						"evpGrantRequestContext", evpGrantRequestContext);
					...
					EVPWorkflowUtil.startWorkflowInstance(
						...
						evpGrantRequest.getUserId(),
						EVPGrantRequest.class.getName(),
						evpGrantRequest.getEvpGrantRequestId(), evpGrantRequest,
					...
					WorkflowInstanceLinkLocalServiceUtil.
						addWorkflowInstanceLink(
							...
							evpGrantRequest.getSubsidiaryGroupId(),
							EVPGrantRequest.class.getName(),
							evpGrantRequest.getEvpGrantRequestId(),
							CounterLocalServiceUtil.increment());
				}
